...
void UAnimNotifyState_AttackCollision::NotifyTick(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation, float FrameDeltaTime, const FAnimNotifyEventReference& EventReference)
{
...
TArray<AActor*> UAnimNotifyState_AttackCollision::CheckCollision(USkeletalMeshComponent* MeshComp, TArray<FVector>& OutHitLocations)
{
	...
	if (AttackCollisionShape.ShapeType == EShape::Box)
	{
		...
		UQVSystemLibrary::DrawDebugBox(MeshComp->GetWorld(), ShapeTransform.GetLocation(), FVector(AttackCollisionShape.HalfExtentX,
			AttackCollisionShape.HalfExtentY, AttackCollisionShape.HalfExtentZ), FColor::Red, FRotator(), 3.f);
	}
